...
- `adapters/affect/` — the runtime affect adapter package. `ClassifierAffectDetector` is the WP-104 production detector and preserves the existing graph-facing contract.
...
10. **Wiring uses a safe backend policy.** `composition/bootstrap.py` supports the production `ClassifierAffectDetector` and a deterministic `DevelopmentAffectDetector`. Clean clones default to `development` until a trained artifact is present; explicitly selecting `classifier` attempts the persisted artifact and falls back to `development` when the artifact cannot be loaded. The graph boundary remains unchanged. *Pass:* runtime construction never requires an uncommitted artifact merely to start, and a valid classifier artifact is used when available.
...
13. **A new integration test exists and passes**, exercising the affect node inside the full graph (`pipeline/graph.py`) with the real detector on at least one real short audio clip, and asserting the resulting decision-trace row's `affect_level` field (§8.3 of `techdocs/SPEC.md`) is populated with a value from the allowed set. Location: `tests/integration/test_ml_affect_integration.py`. *Pass:* `pytest tests/integration/test_ml_affect_integration.py` exits 0; the integration test uses the real `ClassifierAffectDetector` adapter and exercises the full graph/trace boundary with a controlled persisted test artifact.
...
15. **The detector rejects bad input but survives runtime backend failure.** `audio=None` or `sample_rate<=0` raises `ValueError` before feature extraction. A runtime classifier failure (including a missing/corrupt artifact during a turn) is caught at the graph boundary and degrades the affect level to the defined safe fallback, `Low`, so the dialogue turn is not dropped. *Pass:* unit tests cover invalid input, invalid detector output, and detector exceptions.
...
5. **Model artifact missing or corrupted at host startup** — `composition/bootstrap.py` must fail explicitly when the required WP-104 classifier artifact cannot be loaded. WP-104 does not ship a development detector or silent fallback, preventing a missing artifact from being mistaken for a functioning affect model.
...
- **Existing code this package builds on, unmodified:** `pipeline/nodes/affect.py` (`make_affect_node`, `ALLOWED_AFFECT_LEVELS`), `pipeline/state.py` (`DialogueState`), and `config/settings.py` (`audio_sample_rate_hz`). `ClassifierAffectDetector` preserves the existing graph-facing detector contract.
