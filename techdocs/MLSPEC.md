...
15. **The detector degrades to a defined error, not a silent default, on bad input.** `audio=None` or `sample_rate<=0` raises `ValueError` before openSMILE is invoked: `audio=None` or `sample_rate<=0` raises `ValueError` before openSMILE is invoked. Audio shorter than openSMILE's minimum analysable window is caught and raises a named exception (not a raw library traceback) rather than silently returning a default level. *Pass:* unit tests (criterion 12) exercise both cases and assert the specific exception type, not just "an exception was raised."
